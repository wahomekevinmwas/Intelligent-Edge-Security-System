...
// STAGE 2 - Simulated real-time clock using millis()
// START_HOUR sets the demo start hour. When DS3231 RTC is connected,
// replace getSimulatedHour() with: return rtc.now().hour();
#define START_HOUR 8
int getSimulatedHour() {
  unsigned long elapsed = millis() - windowStart;
  int hoursElapsed = (int)(elapsed / 3600000UL);
  return (START_HOUR + hoursElapsed) % 24;
}

int getSimulatedMinute() {
  unsigned long elapsed = millis() - windowStart;
  unsigned long totalMinutes = elapsed / 60000UL;
...
void runML() {
  int hour         = getSimulatedHour();               // FIX: millis() simulated clock
  int resetState   = (digitalRead(resetButton) == LOW) ? 1 : 0;  // FIX: actual pin read
  ...
  float features[NUM_FEATURES] = {
    ...
    (float)hour,           // simulated time context (START_HOUR + elapsed)
    (float)resetState,     // FIX: actual digitalRead(A1) not derived flag
...
void sendSerialJSON() {
  ...
  Serial.print(",\"sim_minute\":");
  Serial.print(getSimulatedMinute());
  Serial.println("}");
}
